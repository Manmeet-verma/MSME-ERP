...
import { db, quotationsTable, clientsTable, productsTable, auditLogsTable, usersTable, quotationItemsTable } from "@workspace/db";
...
reportsRouter.get("/reports/dashboard", requireAuth, async (req, res) => {
  const [totalQ] = await db.select({ count: count() }).from(quotationsTable);
  const [totalC] = await db.select({ count: count() }).from(clientsTable);
  const [totalP] = await db.select({ count: count() }).from(productsTable);
  ...
  const monthlyQ = await db
    ...
    .from(quotationsTable)
    .where(gte(quotationsTable.createdAt, startOfMonth));

  const recentRows = await db.select().from(quotationsTable)
    .orderBy(sql`${quotationsTable.createdAt} DESC`)
    .limit(5);

  const recent = await Promise.all(recentRows.map(async q => {
  ...
  res.json({
    ...
    approvedValue,
    conversionRate,
    thisMonthQuotations: Number(monthlyQ[0]?.count ?? 0),
    thisMonthValue: Number(monthlyQ[0]?.value ?? 0),
...
reportsRouter.get("/reports/monthly", requireAuth, async (req, res) => {
  const months: Array<{
    ...
    quotationCount: number; totalValue: number; approvedValue: number; approvedCount: number;
  }> = [];
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    const [all] = await db.select({ count: count(), value: sum(quotationsTable.total) })
      .from(quotationsTable)
      .where(and(gte(quotationsTable.createdAt, start), lt(quotationsTable.createdAt, end)));

    const [approved] = await db.select({ count: count(), value: sum(quotationsTable.total) })
      .from(quotationsTable)
      .where(and(
...
reportsRouter.get("/reports/top-products", requireAuth, async (req, res) => {
  const rows = await db
    ...
    .where(sql`${quotationItemsTable.productId} IS NOT NULL`)
...
reportsRouter.get("/audit-logs", requireAuth, requireAdmin, async (req, res) => {
  ...
  const users = userIds.length > 0
    ? await db.select().from(usersTable).where(sql`${usersTable.id} = ANY(ARRAY[${sql.raw(userIds.join(","))}])`)
    : [];
