  ...
  try {
    ...
    try {
      ...
      if (err?.code === 9 || err?.code === "FAILED_PRECONDITION") {
        ...
        const fallbackSnap = await db().collection("leads").where("organizationId", "==", orgId).limit(FIRESTORE_HARD_CAP).get();
        let fallbackRows = fallbackSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
        if (status) fallbackRows = fallbackRows.filter((r: any) => r.status === status);
        if (priority) fallbackRows = fallbackRows.filter((r: any) => r.priority === priority);
        if (source) fallbackRows = fallbackRows.filter((r: any) => r.source === source);
        fallbackRows.sort((a: any, b: any) => (b.createdAt ?? "").localeCompare(a.createdAt ?? ""));
        if (search) {
        ...
        const paged = fallbackRows.slice((pageNum - 1) * pageSize, pageNum * pageSize);
        res.json({ data: paged.map((r: any) => formatLead(r.id, r)), total, totalPages, page: pageNum });
        return;
      }
      throw err;
    }
