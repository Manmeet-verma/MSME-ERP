...
export async function requireAuth(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const memberQuery = db.collection("organization_members")
      .where("userId", "==", decoded.userId)
      .where("organizationId", "==", decoded.activeOrgId)
      .limit(1);
    let memberSnap: FirebaseFirestore.QuerySnapshot;
    ...
    try {
      ...
      if (err?.code === 9 || err?.code === "FAILED_PRECONDITION") {
        ...
        indexFailed = true;
        memberSnap = { empty: true, docs: [] } as any;
      } else {
        throw err;
      }
    }
    ...
    if (memberSnap.empty) {
      if (indexFailed) {
        // Index missing — allow access as viewer; real role will resolve once index is deployed
